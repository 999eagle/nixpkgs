...
, nixosTests
  ...
  #   * pick the latest tag
  #   * update .invidious.version, .invidious.version_string, and .invidious.hash
...
crystal.buildCrystalPackage rec {
  ...
  src = fetchFromGitea {
    ...
    rev = if versions.invidious ? rev then versions.invidious.rev else "v${version}";
  ...
  postPatch =
    let
      ...
      versionString = lib.splitString "-" versions.invidious.version_string;
      date = builtins.elemAt versionString 0;
      commit = builtins.elemAt versionString 1;
    in
    ''
